...
class ShopifyInstanceEpt(models.Model):
    ...
    @api.model
    def set_shopify_order_status_ids(self):
        instances = self.search([('shopify_order_status_ids', '=', False)])
        if instances:
            order_status = self.env.ref('shopify_ept.unshipped')
            instances.write({'shopify_order_status_ids': [(6, 0, [order_status.id])]})

    @api.model
    def set_shopify_import_after_date(self):
        ...
        for instance in instances:
            if not instance.import_order_after_date:
                order = sale_order_obj.search([('shopify_instance_id', '=', instance.id)],
    ...
    import_order_after_date = fields.Datetime(help="Connector only imports those orders which have created after a "
                                                   "given date.", default=set_shopify_import_after_date)
    ...
    def prepare_shopify_shop_url(self, host, api_key, password):
        ...
        shop = host.split("//")
        if len(shop) == 2:
            shop_url = shop[0] + "//" + api_key + ":" + password + "@" + shop[1] + "/admin/api/2021-01"
        else:
            shop_url = "https://" + api_key + ":" + password + "@" + shop[0] + "/admin/api/2021-01"
